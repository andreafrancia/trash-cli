# `trash-list --orphans` — Acceptance Scenarios

(Originally implemented under the name `--show-non-trashinfo`; renamed to `--orphans`
after clarifying that "non-trashinfo" actually refers to a different, not-yet-built
feature: files inside `info/` whose name doesn't end in `.trashinfo`. Behavior described
below is unchanged — only the flag name and identifiers were corrected.)
...
tests are plain pytest in `tests/test_list/cmd/test_orphans.py`.

## Locked-in behavior

- `--orphans` is **exclusive**: when passed, only orphan paths (files under a
...
- `--size` and/or `--files` combined with `--orphans` are ignored (they have
...
- Warning text lists ignored flags in alphabetical order, regardless of the order they
  ...
  - `trash-list: --files and --size are ignored when --orphans is used`
...
When:
    $ trash-list --orphans --trash-dir /disk1/.Trash-1000
