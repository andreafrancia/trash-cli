...
class TestParseTrashInfo(unittest.TestCase):
    def test_it_should_parse_date(self):
        ...
        parser.parse_trashinfo(b'[Trash Info]\n'
                               b'Path=foo\n'
                               b'DeletionDate=1970-01-01T00:00:00\n')
...
class TestParseDeletionDate(unittest.TestCase):
    def test1(self):
        assert parse_deletion_date(b'DeletionDate=2000-12-31T23:59:58') == \
    ...
    def test_two_deletion_dates(self):
        assert parse_deletion_date(b'DeletionDate=2000-01-01T00:00:00\n'
                                   b'DeletionDate=2000-12-31T00:00:00\n') == \
...
class Test_maybe_parse_deletion_date(unittest.TestCase):
    ...
    def test_on_trashinfo_with_date_parse_to_date(self):
        from datetime import datetime
        example_date_as_string = b'2001-01-01T00:00:00'
...
def a_trashinfo_without_deletion_date():  # type: () -> TrashInfoContent
    return (b"[Trash Info]\n"
            b"Path=foo.txt\n")


def make_trashinfo(date,  # type: binary_type
                   ):  # type: (...) -> TrashInfoContent
    return (b"[Trash Info]\n"
            b"Path=foo.txt\n"
            b"DeletionDate=%s" % date)
